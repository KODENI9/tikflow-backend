...
export const rebuildStats = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { AnalyticsService } = require('../services/analytics.service');
        const stats = await AnalyticsService.rebuildStatsFromScratch();
        res.status(200).json({ success: true, message: "Statistiques reconstruites avec succès.", data: stats });
    } catch (error) {
        next(error);
    }
};
